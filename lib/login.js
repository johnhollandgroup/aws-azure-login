...
const states = [{
    ...
    },
    {
        ...
        async handler(page, _selected, _noPrompt, _defaultUsername, _defaultPassword, customDelayModifier, rememberMe) {
            if (rememberMe) {
...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, customDelayModifier) {
        ...
        const samlResponse = await this._performLoginAsync(loginUrl, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, profile.azure_default_username, profile.azure_default_password, enableChromeSeamlessSso, customDelayModifier, profile.azure_default_remember_me);
     ...
     * @param {number} [customDelayModifier] - The delay multiplier for login waits
     * @param {bool} [rememberMe] - Enable remembering the session
     * @returns {Promise.<string>} The SAML response.
     * @private
     */
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword, enableChromeSeamlessSso, customDelayModifier, rememberMe) {
